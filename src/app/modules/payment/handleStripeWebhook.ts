...
const handleStripeWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    if (eventType === "checkout.session.completed") {
      ...
      const quote = await Quote.findById(session.metadata?.quote_id).populate("serviceType");
      ...
      const paymentConfirmationEmailTemplate = emailTemplate.sendPaymentConfirmationEmail(quote)
      await emailHelper.sendEmail(paymentConfirmationEmailTemplate)

      // Send admin payment notification email
      const adminPaymentNotificationEmailTemplate = emailTemplate.sendAdminPaymentNotificationEmail(quote)
      await emailHelper.sendEmail(adminPaymentNotificationEmailTemplate)
