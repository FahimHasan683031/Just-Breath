...
// get quotes
 const getQuotes = async (query: Record<string, unknown>) => {
  const quoteQueryBuilder = new QueryBuilder(Quote.find(), query)
    .filter()
    .sort()
    .paginate();
...
// update quote
 const updateQuote = async (id: string, payload: Partial<IQuote>) => {
  const isExist = await Quote.findById(id);
  if (!isExist) {
    throw new ApiError(StatusCodes.NOT_FOUND, 'Quote not found');
  }
 ...
 const sendPaymentLink = async (id: string) => {
  ...
  if (!quote) {
    throw new ApiError(StatusCodes.NOT_FOUND, 'Quote not found');
      ...
      const paymentLinkEmailTemplate = emailTemplate.sendPaymentLinkEmail({data:quote,paymentUrl:`${config.payment_page_link}?quoteId=${id}`,})
